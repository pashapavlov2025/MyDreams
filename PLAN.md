...
Что хранить можно — **реквизиты и указатели**. Уточнено владельцем: нужны не
заметки в свободной форме, а конкретные поля:

- **Номер договора / счёта** (не полный номер карты)
- **Контакт менеджера**: имя, телефон, email
- **Официальный адрес организации** для обращения — почтовый или юридический
- **Как устроен доступ**: СМС на такой-то номер, вход через такое-то
  приложение, только в офисе лично
- Страна и тип счёта
- Где лежат документы (папка, сейф, у нотариуса)
- Оформлен ли наследник/бенефициар по этому счёту
- Свободный комментарий — дополнением, а не вместо полей

Про «как устроен доступ» отдельно: сам факт «вход по СМС на +7…» секретом не
является, но именно он объясняет наследнику, почему без этого номера ничего не
выйдет и что с номером нужно что-то делать в первую очередь.

#### Задачи

- [x] **6.1** Блок реквизитов у счёта: договор, менеджер, адрес,
      способ доступа, комментарий. Все поля необязательные.
      *(проекты — отдельно, если понадобится)*
- [ ] **6.2** Раздел на уровне профиля: контакты (юрист, нотариус, банкир),
      где документы, общая инструкция
- [x] **6.3** Человекочитаемый экспорт (HTML) — **ключевой пункт**.
      Жене не нужен JSON и не нужно приложение: нужен документ, который можно
      открыть и прочитать. Реализована выгрузка `.html` из раздела Settings.
