...
- [x] **2.5.1** Мультиязычность (i18n):
  ...
  - ⚠️ Мелкие недоделки: 3 захардкоженных "Загрузка..." (page.tsx), `lang="ru"` в layout.tsx, formatMoney не учитывает локаль
...
- [ ] **2.5.3** Базовый a11y (~40%):
  - [x] Семантические HTML-теги (`<h1>`–`<h3>`, `<section>`, `<button>`, `<label>`)
  - [x] Focus-стили (`focus:ring-2`) на всех инпутах
  - [ ] aria-labels на icon-only кнопках (есть только на 4 из ~20+)
  - [ ] Семантические landmarks (`<nav>`, `<main>`, `<footer>`)
