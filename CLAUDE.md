...
- **Account**: `{profileId, name, type, currency, icon, bankGroup?, sortOrder, isArchived, metadata?}`
  - `metadata` — реквизиты для близких: договор, контакт менеджера, адрес,
    способ доступа, страна, где документы, наследник, комментарий. Все поля
    опциональные.
...
| `FamilyAccessSection` | Выгрузка человекочитаемой HTML-карты доступа для семьи |
...
## Экспорт для близких (Family access)

Отдельный от бэкапа человекочитаемый экспорт — HTML-файл, который можно
отправить семье без приложения.

`src/lib/familyExport.ts`:

| Функция | Что делает |
|---------|-----------|
| `generateFamilyAccessHtml(ctx)` | Собирает HTML со счетами, балансами и метаданными |
| `deliverFamilyAccessDocument(html, name)` | Web Share на iOS / download на десктопе |
| `familyAccessFilename(date?)` | Имя файла `mydreams-family-access-YYYY-MM-DD.html` |

Что попадает в файл: профиль, дата, дисклеймер, по каждому счёту — название,
банк, валюта, последний баланс, дата обновления и все заполненные поля
`metadata`. Пароли, PIN и seed-фразы не экспортируются.
...
| 6. Доступ для близких | 🔧 | Реквизиты у счётов ✅, HTML-экспорт ✅, дальше: профиль-инструкция, зашифрованный экспорт |
